...
if(isset($_POST['sub'])){    
	...
	if($your_img != ''){
        ...
        {
            move_uploaded_file($_FILES['your_img']['tmp_name'],'../../images/about/'.$your_imgtt);
            $b = "INSERT INTO `tc_about`( `login_id`,`description`, `image`, `date_time`, `date`) VALUES ('$login_id','$description','$your_imgtt','$date_time','$date')";
            mysqli_query($conn,$b);
            echo "<script>alert('Product added Successfully!');</script>";
	        echo "<script>window.location.href='about.php';</script>";
        }
	}else{
		$b = "INSERT INTO `tc_about`( `login_id`,`description`, `date_time`, `date`) VALUES ('$login_id','$description','$date_time','$date')";
        mysqli_query($conn,$b);
        echo "<script>alert('Product added Successfully!');</script>";
	    echo "<script>window.location.href='about.php';</script>";
	}
...
if(isset($_POST['upd'])){
	$id = $_POST['upd'];
	$tt = "SELECT * FROM `tc_about` WHERE `id` = '$id' AND `login_id` = '$login_id'";
...
if(isset($_POST['update']))
{
    ...
    if($your_img != '')
    {
        ...
        else 
        {
             ...
             $tt5 = "SELECT * FROM `tc_about` WHERE `id` = '$id'";
             $ee5 = mysqli_query($conn,$tt5);
             $qq5 = mysqli_fetch_array($ee5);
             unlink('../../images/about/'.$qq5['image']);
             move_uploaded_file($_FILES['your_img']['tmp_name'],'../../images/about/'.$your_imgtt);
             $b = "UPDATE `tc_about` SET `description`='$description',`image`='$your_imgtt',`date_time`='$date_time',`date`='$date' WHERE `id` = '$id' AND `login_id` = '$login_id'";
             mysqli_query($conn,$b);
             echo "<script>alert('Update successfully!');</script>";
             echo "<script>window.location.href='about.php';</script>";
        }
    }else{
        $b = "UPDATE `tc_about` SET `description`='$description',`date_time`='$date_time',`date`='$date' WHERE `id` = '$id' AND `login_id` = '$login_id'";
        mysqli_query($conn,$b);
        echo "<script>alert('Update successfully!');</script>";
        echo "<script>window.location.href='about.php';</script>";
    }
...
if(isset($_POST['del'])){
    $id = $_POST['del'];
    //unlink image from server
    $tt5 = "SELECT * FROM `tc_about` WHERE `id` = '$id' AND `login_id` = '$login_id'";
    $ee5 = mysqli_query($conn,$tt5);
    $qq5 = mysqli_fetch_array($ee5);
    unlink('../../images/about/'.$qq5['image']);

	$tt = "DELETE FROM `tc_about` WHERE `id` = '$id' AND `login_id` = '$login_id'";
	...
	<section class="content">
		<div class="row">
			<div class="col-md-12">
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><b>About Us</b></h3>
					</div>
                            ...
                            {?>
                                <div class="col-md-3" style="">	
                                    <?php 
                                        if($qq['image'] !='') 
                                        { ?>
                                            <img src="../../images/about/<?php echo $qq['image']; ?>" style="width:120px">
                                            <?php 
                                        } else
                                        {
            ...
            <div class="col-md-12">	
				<div class="box">
					<div class="box-header">
						<h3 class="box-title"><b>Manage About Us</b></h3>
					</div>
					<div class="box-body box box-info">
						...
						<table class="table table-bordered example1">
							...
							<tbody>
								<?php
									$vv1 = "SELECT * FROM `tc_about` WHERE `login_id` = '$login_id' ORDER BY id DESC";
								...
								<tr>
									...
									<td>
                                        <?php 
                                            $string = strip_tags($nn1['description']);
                                            $charLen = 35;
									...
									<td>
                                        ...
                                        if($nn1['image'] !='') 
                                        { ?>
                                            <img src="../../images/about/<?php echo $nn1['image']; ?>" style="width:120px">
                                            <?php 
                                        } else
                                        {
                                            echo 'Image not found.';
